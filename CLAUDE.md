...
- **hooks/edge_speak.py** — detached helper for the `edge` engine: synthesizes via `uvx edge-tts` (no install — runs out-of-process, plugin stays stdlib-only), plays the mp3 with `afplay`, and falls back to local `say` on any failure (offline, no `uvx`, timeout, empty result). Reads its payload from the `SIMPLE_TTS_PAYLOAD` env var (so text never appears in `ps`). Launched as its own process group so a priority interrupt kills synthesis + playback together via `os.killpg`. **Audio cache**: the mp3 is stored under `~/.claude/simple-tts-audio-cache/` content-addressed by `sha256(engine, voice, rate, text)` — a repeated phrase with the same voice plays straight from cache and skips synthesis (~100×+ faster). Synthesis writes a `.synthtmp-*.mp3` temp inside the cache dir and is `os.replace`d into place on success (atomic, no partial reads); the temp is always unlinked in `finally`, stale temps from a killed synth are swept (older than `TMP_MAX_AGE`), and the cache is pruned to `MAX_CACHE_ENTRIES` (oldest-by-mtime; hits `os.utime`-touch for approx-LRU). The `say` fallback is never cached
...
- **hooks/message_display.py** — MessageDisplay hook (tag mode): redacts the hidden `<!-- TTS: ... -->` marker from the on-screen display via `hookSpecificOutput.displayContent`. Display-only — the transcript keeps the marker so the Stop hook still speaks it. Field-name-agnostic (the MessageDisplay stdin schema is undocumented): it locates the marker anywhere in the JSON and returns the cleaned text; emits nothing when no marker is present. This is what makes tag mode marker-free in the console (some Claude Code versions render HTML comments as visible text)
