...
- **skills/tts/SKILL.md** — `/tts on|off|status` toggles `"enabled"` in the config (mute without uninstalling); `/tts cache [stats|prune|clear]` manages the audio cache via `cache_cli.py` (resolves the script path whether installed as a plugin or run from the repo)
