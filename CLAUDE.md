...
- **overlay/** — the **Knight Rider scanner overlay** (optional GUI daemon, not on the hook path so the core stays stdlib-only). `kitt_frame.py` renders a frame (PIL, additive-glow LEDs) for one of three modes; `kitt_state.py` (stdlib, unit-tested) picks the mode from simple-tts state — **speak** when the TTS PID in `simple-tts-state.json` is alive (mirrors `_is_our_tts`), **think** when `simple-tts-busy` is `1`, **idle** otherwise, `None` when `knight_rider` is off; `kitt_overlay.py` is a transparent, click-through, always-on-top Cocoa `NSPanel` (per screen, `CanJoinAllSpaces` + `FullScreenAuxiliary` so it floats over a native-fullscreen terminal). **Performance:** each mode's frames are rendered once (one seamless loop period) into `CGImage`s and handed to Core Animation as a `CAKeyframeAnimation` (discrete) on `layer.contents` — the window server cycles them on the GPU, so steady-state CPU is ~0.5% (vs ~27% for per-frame PIL render + PNG). The only recurring work is a 0.25 s state-check timer that swaps the animation on mode change. Deps (pyobjc Cocoa+Quartz + Pillow) and autostart via `install_overlay.sh` (copies to `~/.claude/simple-tts-overlay/`, writes a LaunchAgent); `uninstall_overlay.sh` reverses it
